...
<div class="relative inline-block">
  <!-- Image Preview -->
  <img bind:this={imageRef} {src} {alt} class={className} style={styleString} />
  ...
  {#if showPanel}
    <div
      class="absolute bottom-full left-0 mb-2 w-72 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-background)] p-4 shadow-xl"
    >
      <!-- Tabs -->
      <div class="mb-4 flex gap-1 border-b border-[var(--ui-border)]">
        <button
          onclick={() => (activeTab = "filters")}
          class="px-3 py-1.5 text-xs {activeTab === 'filters'
            ? 'border-b-2 border-[var(--ui-primary)] font-medium'
            : ''}">Filters</button
        >
        <button
          onclick={() => (activeTab = "transform")}
          class="px-3 py-1.5 text-xs {activeTab === 'transform'
            ? 'border-b-2 border-[var(--ui-primary)] font-medium'
            : ''}">Transform</button
        >
        <button
          onclick={() => (activeTab = "effects")}
          class="px-3 py-1.5 text-xs {activeTab === 'effects'
            ? 'border-b-2 border-[var(--ui-primary)] font-medium'
            : ''}">Effects</button
        >
      </div>
        ...
        <div class="space-y-3">
          <div>
            <label class="flex justify-between text-xs"
              >Brightness <span>{filters.brightness}</span></label
            >
            <input
              type="range"
              min="0"
          ...
          <div>
            <label class="flex justify-between text-xs"
              >Contrast <span>{filters.contrast}</span></label
            >
            <input
              type="range"
              min="0"
          ...
          <div>
            <label class="flex justify-between text-xs"
              >Saturation <span>{filters.saturate}</span></label
            >
            <input
              type="range"
              min="0"
              max="2"
              step="0.1"
              bind:value={filters.saturate}
              class="w-full"
            />
          </div>
          <div>
            <label class="flex justify-between text-xs"
              >Grayscale <span>{filters.grayscale}</span></label
            >
            <input
              type="range"
              min="0"
          ...
          <div>
            <label class="flex justify-between text-xs"
              >Sepia <span>{filters.sepia}</span></label
            >
            <input
              type="range"
              min="0"
          ...
          <div>
            <label class="flex justify-between text-xs"
              >Blur <span>{filters.blur}px</span></label
            >
            <input
              type="range"
              min="0"
      ...
      {:else if activeTab === "transform"}
        <div class="space-y-3">
          <div>
            <label class="flex justify-between text-xs"
              >Rotate <span>{transform.rotate}°</span></label
            >
            <input
              type="range"
              min="0"
      ...
      {:else}
        <div class="space-y-3">
          <div>
            <label class="flex justify-between text-xs"
              >Border Radius <span>{effects.borderRadius}px</span></label
            >
            <input
              type="range"
              min="0"
              max="50"
              step="1"
              bind:value={effects.borderRadius}
              class="w-full"
            />
          </div>
          <label class="flex items-center gap-2 text-xs">
          ...
          {#if effects.shadow}
            <div>
              <label class="flex justify-between text-xs"
                >Shadow Blur <span>{effects.shadowBlur}px</span></label
              >
              <input
                type="range"
                min="0"
