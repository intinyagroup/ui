...
import type { Effect, EffectType } from "@intinyagroup/motion";
...
export function getFilterString(
  ...
  for (const effect of effects) {
    if (effect.fromFrame !== undefined && currentFrame < effect.fromFrame)
      continue;
    ...
    switch (effect.type) {
      ...
      case "drop-shadow":
        filters.push(
          `drop-shadow(${effect.params.x ?? 0}px ${effect.params.y ?? 4}px ${effect.params.blur ?? 4}px ${effect.params.color ?? "rgba(0,0,0,0.3)"})`,
        );
        break;
    }
  }
...
export function getTransformString(
  ...
  for (const effect of effects) {
    ...
    const progress = Math.min(
      1,
      (currentFrame - (effect.fromFrame ?? 0)) /
        Math.max(1, (effect.toFrame ?? 30) - (effect.fromFrame ?? 0)),
    );
    switch (effect.type) {
      case "zoom-in":
        transforms.push(
          `scale(${1 + (effect.params.amount ?? 0.5) * progress})`,
        );
        break;
      case "zoom-out":
        transforms.push(
      ...
      case "pan-left":
        transforms.push(
          `translateX(-${(effect.params.amount ?? 50) * progress}px)`,
        );
        break;
      case "pan-right":
        transforms.push(
      ...
      case "pan-up":
        transforms.push(
          `translateY(-${(effect.params.amount ?? 50) * progress}px)`,
        );
        break;
      case "pan-down":
        transforms.push(
      ...
      case "rotate":
        transforms.push(
          `rotate(${(effect.params.degrees ?? 360) * progress}deg)`,
        );
        break;
      case "shake":
        transforms.push(
          `translate(${Math.sin(currentFrame * 0.5) * (effect.params.intensity ?? 5)}px, ${Math.cos(currentFrame * 0.7) * (effect.params.intensity ?? 5)}px)`,
        );
        break;
      case "bounce":
        transforms.push(
          `translateY(-${Math.abs(Math.sin(progress * Math.PI * 3)) * (effect.params.height ?? 20)}px)`,
        );
        break;
      case "pulse":
        transforms.push(
          `scale(${1 + Math.sin(currentFrame * 0.2) * (effect.params.amount ?? 0.1)})`,
        );
        break;
    }
  }
