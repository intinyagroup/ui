...
import type { Transition, TransitionType } from "@intinyagroup/motion";
...
export function getTransitionStyles(
  ...
  switch (transition.type) {
    case "fade":
    case "cross-fade":
      return { opacity: String(p) };
    case "slide-left":
      return { transform: `translateX(${(1 - p) * 100}%)`, opacity: String(p) };
    case "slide-right":
      return { transform: `translateX(${(p - 1) * 100}%)`, opacity: String(p) };
    case "slide-up":
      return { transform: `translateY(${(1 - p) * 100}%)`, opacity: String(p) };
    case "slide-down":
      return { transform: `translateY(${(p - 1) * 100}%)`, opacity: String(p) };
    case "wipe-left":
    ...
    case "dissolve":
      return { filter: `blur(${(1 - p) * 10}px)`, opacity: String(p) };
    case "blur":
      return { filter: `blur(${(1 - p) * 20}px)`, opacity: String(p) };
    case "zoom":
      return { transform: `scale(${0.5 + p * 0.5})`, opacity: String(p) };
    case "rotate":
      return {
        transform: `rotate(${(1 - p) * 180}deg) scale(${0.5 + p * 0.5})`,
        opacity: String(p),
      };
    case "push-left":
