  ...
  const pricingCardVariants = tv({
    ...
    variants: {
      highlighted: {
        ...
        false:
          "border-[var(--ui-border)] hover:-translate-y-0.5 hover:shadow-md",
      },
    },
    defaultVariants: {
      highlighted: false,
...
>
  {#if highlighted}
    <div
      class="absolute inset-x-0 -top-px h-px bg-gradient-to-r from-transparent via-[var(--ui-primary)] to-transparent"
    ></div>
  {/if}
  ...
  {#if features.length > 0}
    <ul class="flex flex-1 flex-col gap-3">
      {#each features as feature}
        <li
          class="flex items-start gap-2 text-sm text-[var(--ui-muted-foreground)]"
        >
          <svg
