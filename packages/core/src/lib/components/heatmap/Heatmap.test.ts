...
describe("Heatmap", () => {
  it("renders a cell per data point with an accessible label", () => {
    render(Heatmap, { data });
    expect(
      screen.getByRole("img", { name: "Mon, wk1: 1" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("img", { name: "Mon, wk2: 5" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("img", { name: "Tue, wk1: 3" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders cells with no data without a role or color", () => {
    ...
    expect(
      container.querySelectorAll('div[class*="rounded-[4px]"]').length,
    ).toBe(4);
  });
