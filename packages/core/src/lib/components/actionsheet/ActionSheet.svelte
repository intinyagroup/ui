...
<svelte:window
  onkeydown={(e) => e.key === "Escape" && open && ((open = false), onclose?.())}
/>
...
{#if open}
  ...
  >
    ...
    <div
      ...
      style="animation: intinya-action-sheet-fade 200ms ease-out"
      onclick={() => {
        open = false;
        onclose?.();
      }}
    ...
    <div
      ...
      style="animation: intinya-action-sheet-slide 300ms cubic-bezier(0.32, 0.72, 0, 1)"
    >
      <div
        class="overflow-hidden rounded-2xl bg-[var(--ui-background)] shadow-[var(--ui-shadow-lg)]"
      >
        {#if title}
          <div
            class="border-b border-[var(--ui-border)] px-4 pb-3 pt-4 text-center"
          >
...
<style>
  @keyframes intinya-action-sheet-fade {
    from {
      opacity: 0;
    }
    to {
  ...
  @keyframes intinya-action-sheet-slide {
    from {
