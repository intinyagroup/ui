        ...
        <div class={cn("min-w-0 flex-1", commentClass)}>
          <div class="flex flex-wrap items-center gap-x-2">
            ...
            {#if canDelete(comment.author)}
              <span
                class="rounded bg-[var(--ui-muted)] px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-[var(--ui-muted-foreground)]"
              >
                you
              </span>
            {/if}
          </div>
          <p class="mt-0.5 text-sm text-[var(--ui-foreground)]">
            {comment.text}
          </p>
