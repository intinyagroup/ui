<script lang="ts" module>
  ...
  export type NumberInputProps = WithElementRef<
    Omit<HTMLInputAttributes, "type" | "size" | "value"> & {
      value?: number;
      min?: number;
      max?: number;
      step?: number;
...
<script lang="ts">
  ...
  const inputId = $derived(
    restProps.id ?? `number-input-${Math.random().toString(36).slice(2, 9)}`,
  );
  ...
  function decrement() {
    ...
    if (next !== value) {
      value = next;
      onchange?.(
        new CustomEvent("change", { detail: { value } }) as unknown as Event & {
...
>
  ...
  <div class="relative flex-1 flex items-center justify-center">
    <input
      ...
      {disabled}
      {placeholder}
      class="h-full w-full border-0 bg-transparent text-center outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      style="min-width: 0;"
      oninput={handleInput}
      onblur={handleBlur}
    ...
    {#if unit}
      <span
        class="pointer-events-none absolute right-1.5 text-[var(--ui-muted-foreground)] text-xs select-none"
        aria-hidden="true"
      >
        {unit}
      </span>
