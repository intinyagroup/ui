<script lang="ts">
  ...
  let {
    ...
    content?: Snippet<
      [{ currentStep: number; next: () => void; prev: () => void }]
    >;
  } = $props();
    ...
    <div
      class={cn(
        ...
        orientation === "vertical"
          ...
          : "flex-col items-center gap-2 flex-1",
    ...
    >
      ...
      <div class="relative flex items-center gap-3">
        ...
        {#if orientation === "vertical" && index < steps.length - 1}
          <div
            class="absolute left-4 top-10 w-0.5 h-full {status === 'completed'
              ? 'bg-[var(--ui-primary)]'
              : 'bg-[var(--ui-border)]'}"
          ></div>
        {/if}
        ...
        <button
          ...
          class={cn(
            ...
            status === "completed"
              ...
              : "",
            status === "current"
              ? "bg-[var(--ui-primary)]/10 text-[var(--ui-primary)] ring-2 ring-[var(--ui-primary)]/30"
              : "",
            status === "upcoming"
              ? "bg-[var(--ui-secondary)] text-[var(--ui-muted-foreground)]"
              : "",
          )}
        >
          {#if status === "completed"}
        ...
        <div class={cn(orientation === "vertical" ? "" : "hidden sm:block")}>
          ...
          {#if step.description}
            <div class="text-xs text-[var(--ui-muted-foreground)]/70 mt-0.5">
              {step.description}
            </div>
          {/if}
        </div>
      ...
      {#if orientation === "horizontal" && index < steps.length - 1}
        <div
          class="flex-1 h-0.5 mt-4 mx-2 {status === 'completed'
            ? 'bg-[var(--ui-primary)]'
            : 'bg-[var(--ui-border)]'}"
        ></div>
      {/if}
