  ...
  {#if !disabled}
    <div
      ...
      style="height: {pulling || refreshing
        ? pullDistance
        : 0}px; transition: {pulling || refreshing
        ? 'none'
    ...
    >
      ...
      >
        <span
          ...
          style={refreshing
            ? "animation: intinya-ptr-spin 800ms linear infinite"
            : ""}
        >
          <svg
            viewBox="0 0 24 24"
            ...
            class="h-5 w-5"
            style="transform: rotate({Math.min(pullDistance / THRESHOLD, 1) *
              360}deg)"
          >
            <path d="M12 2v4" />
