<script lang="ts">
  ...
  function onMotion(event: DeviceMotionEvent) {
    ...
    const beta = clamp(
      Math.asin(clamp(y / norm, -1, 1)) * (180 / Math.PI),
      -180,
      180,
    );
    const gamma = clamp(
      Math.asin(clamp(x / norm, -1, 1)) * (180 / Math.PI),
      -90,
      90,
    );
    if (!supported) supported = true;
...
>
  <div class="flex items-center justify-between gap-3">
    <div class="text-sm font-medium">
      ...
      {#if mode === "mouse"}
        <span
          class="ml-2 rounded-full bg-[var(--ui-secondary)] px-2 py-0.5 text-xs"
          >Simulated (mouse)</span
        >
      {/if}
      {#if permissionState === "denied"}
        <span
          class="ml-2 rounded-full bg-red-100 px-2 py-0.5 text-xs text-red-700"
