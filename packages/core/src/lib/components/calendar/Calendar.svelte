<script lang="ts">
  ...
  function isSameDate(a: CalendarDate | null, b: CalendarDate): boolean {
  ...
  function goToday() {
    ...
    selectDate({ year: today.getFullYear(), month: today.getMonth(), day: today.getDate() });
  }
</script>
...
<div class={cn('inline-flex flex-col rounded-xl border border-[var(--ui-border)] bg-[var(--ui-card)] p-4 shadow-sm', className)}>
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="flex items-center gap-2">
      ...
      <Button variant="ghost" size="sm" onclick={goToday} {disabled} class="text-xs h-6 px-2">
        Today
      </Button>
    </div>
  ...
  <div class="grid grid-cols-7 gap-1 mb-2">
    {#each dayNames as dayName}
      <div class="text-center text-[11px] font-medium text-[var(--ui-muted-foreground)] py-1">
        {dayName}
      </div>
    {/each}
  </div>
    ...
    {#each calendarDays as { day, currentMonth, date } (date.year + '-' + date.month + '-' + date.day)}
      ...
      <button
        ...
        class="flex items-center justify-center size-9 rounded-lg text-sm transition-colors cursor-pointer
          ...
          {isDisabledDate ? 'opacity-40 cursor-not-allowed' : ''}
