...
describe("validators.required", () => {
  ...
  it("rejects undefined", () => {
    expect(validators.required().validate(undefined)).toBe(
  ...
  it("uses custom message", () => {
    expect(validators.required("Name is required").validate("")).toBe(
      "Name is required",
    );
  });
});
...
describe("validators.minLength", () => {
  it("rejects values shorter than min", () => {
    expect(validators.minLength(3).validate("ab")).toBe(
  ...
  it("uses custom message", () => {
    expect(validators.minLength(3, "Too short").validate("ab")).toBe(
      "Too short",
    );
  });
});
...
describe("validators.email", () => {
  it("rejects invalid emails", () => {
    expect(validators.email().validate("not-an-email")).toBe(
      "Please enter a valid email",
    );
    expect(validators.email().validate("a@b")).toBe(
      "Please enter a valid email",
    );
    expect(validators.email().validate("@b.com")).toBe(
      "Please enter a valid email",
    );
  });
...
describe("createFormValidator", () => {
  ...
  it("validateField returns error for a failing field", () => {
    ...
    expect(validator.validateField("email", "nope")).toBe(
      "Please enter a valid email",
    );
  });
