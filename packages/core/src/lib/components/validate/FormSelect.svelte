...
<div class={cn("grid gap-1.5", className)}>
  {#if label}
    <label for={restProps.id as string | undefined} class="text-sm font-medium"
      >{label}</label
    >
  {/if}
  <div class="relative">
    <select
      ...
      class={cn(
        "h-8 w-full min-w-0 appearance-none rounded-lg border border-input bg-input py-1 pr-8 pl-2.5 text-base transition-colors outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 md:text-sm disabled:bg-input/80 aria-invalid:border-destructive/50",
        className,
      )}
      {...restProps}
    >
