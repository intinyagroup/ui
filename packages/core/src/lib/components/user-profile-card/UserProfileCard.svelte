...
<div
  class={cn(
    "overflow-hidden rounded-xl border border-[var(--ui-border)] bg-[var(--ui-card)]",
    className,
  )}
>
  <!-- Cover -->
  ...
  <div class="flex flex-col items-center px-6 pb-6 -mt-10">
    ...
    {#if bio}
      <p
        class="mt-2 text-center text-sm leading-relaxed text-[var(--ui-muted-foreground)]"
      >
        {bio}
      </p>
    {/if}
    ...
    {#if stats.length > 0}
      <div
        class="mt-4 grid w-full grid-cols-3 divide-x divide-[var(--ui-border)] rounded-lg border border-[var(--ui-border)] bg-[var(--ui-muted)]/50"
      >
        {#each stats as stat (stat.label)}
          <div class="flex flex-col items-center px-2 py-2.5">
            <span class="text-base font-bold text-[var(--ui-foreground)]"
              >{stat.value}</span
            >
