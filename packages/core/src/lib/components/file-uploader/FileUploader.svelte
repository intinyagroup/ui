...
<div class={cn("grid gap-4", className)}>
  ...
  <div
    class="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed p-8 transition-colors cursor-pointer
      ...
      : 'border-[var(--ui-border)] hover:border-[var(--ui-primary)]/50 hover:bg-[var(--ui-secondary)]/30'}
  ...
  >
    ...
    <div class="text-center">
      ...
      <p class="text-xs text-[var(--ui-muted-foreground)] mt-1">
        {multiple ? `Up to ${maxFiles} files` : "1 file"} · Max {formatSize(
          maxSize,
        )}
      </p>
    </div>
  ...
  {#if files.length > 0}
    <div class="grid gap-2">
      {#each files as item (item.id)}
        ...
        <div
          class="flex items-center gap-3 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)] p-3"
        >
          {#if item.preview}
            <img
              src={item.preview}
          ...
          {:else}
            <div
              class="flex items-center justify-center size-10 rounded-md bg-[var(--ui-secondary)]"
            >
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-[var(--ui-foreground)] truncate">
              {item.file.name}
            </p>
            ...
            {#if item.status === "uploading" && item.progress !== undefined}
              <div
                class="mt-1 h-1.5 rounded-full bg-[var(--ui-secondary)] overflow-hidden"
              >
                <div
            ...
            {/if}
            {#if item.error}
              <p class="text-xs text-[var(--ui-destructive)] mt-1">
                {item.error}
              </p>
            {/if}
          </div>
          ...
          <div class="shrink-0">
            {#if item.status === "success"}
              <span class="text-xs text-[var(--ui-success)] font-medium"
                >✓ Done</span
              >
            {:else}
              <button
