...
<li class={cn("list-none", className)}>
  ...
  {:else}
    <div
      class="flex min-h-12 w-full items-center gap-3 bg-transparent px-4 py-2.5 text-left"
    >
      {@render content()}
    </div>
  {/if}
...
{#snippet content()}
  ...
  <span class="flex min-w-0 flex-1 flex-col justify-center">
    <span
      class="truncate text-[15px] font-medium leading-tight text-[var(--ui-foreground)]"
    >
      {title}
    </span>
    {#if description}
      <span
        class="truncate text-[13px] leading-snug text-[var(--ui-muted-foreground)]"
      >
        {description}
      </span>
    {/if}
