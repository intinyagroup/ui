...
export async function validateVideo(
  ...
  return new Promise((resolve) => {
    ...
    v.onerror = () => {
      ...
      resolve({
        compatible: false,
        reason: `Cannot decode ${file.type || "unknown"}`,
      });
    };
...
export async function uploadWithOptimistic(
  ...
  try {
    const cloudUrl = await provider.upload(file, (p) =>
      onStateChange?.({ status: "uploading", progress: p, blobUrl }),
    );
    ...
    onStateChange?.({ status: "error", progress: 0, blobUrl, error: msg });
    return { status: "error", progress: 0, blobUrl, error: msg };
  }
}
