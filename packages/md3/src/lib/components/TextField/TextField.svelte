<script lang="ts">
  ...
  const inputId = $derived(
    id ||
      `md3-textfield-${label?.replace(/\s+/g, "-").toLowerCase() || "input"}`,
  );
  const hasValue = $derived(value.length > 0);
  const isFloated = $derived(focused || hasValue);
  const showError = $derived(!!error);
  const activeColor = $derived(
    showError ? "var(--ui-error, #b3261e)" : "var(--ui-primary, #6750a4)",
  );
  const outlineColor = $derived(
    showError
      ? "var(--ui-error, #b3261e)"
      : focused
        ? "var(--ui-primary, #6750a4)"
        : "var(--ui-outline, #79747e)",
  );
  const indicatorClass = $derived(
    focused ? "w-full" : "w-0 left-1/2 -translate-x-1/2",
  );
  const labelClass = $derived(
    cn(
      ...
      isFloated
        ? "top-0.5 text-sm"
        : (variant === "filled" ? "top-3" : "top-3.5") + " text-base",
      showError
        ? "text-[var(--ui-error, #b3261e)]"
        : focused
          ? "text-[var(--ui-primary, #6750a4)]"
          : "text-[var(--ui-on-surface-variant, #49454f)]",
...
>
  {#if variant === "filled"}
    ...
    <div
      class="relative rounded-t-lg bg-[var(--ui-surface-container-high, #e7e0ec)]"
    >
      <!-- State layer -->
      <span
  ...
  {#if showError && error}
    <p
      id="{inputId}-error"
      class="mt-1 px-4 text-xs text-[var(--ui-error, #b3261e)]"
      role="alert"
    >
      {error}
    </p>
  {:else if helperText}
    <p
      id="{inputId}-helper"
      class="mt-1 px-4 text-xs text-[var(--ui-on-surface-variant, #49454f)]"
    >
      {helperText}
    </p>
  {/if}
