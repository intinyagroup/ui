<script lang="ts" module>
  ...
  export const fabVariants = tv({
    ...
    variants: {
      variant: {
        elevated:
          "bg-[var(--ui-primary)] text-[var(--ui-primary-foreground)] shadow-[var(--ui-shadow-lg)] hover:shadow-[var(--ui-shadow-xl)]",
        surface:
          "bg-[var(--ui-secondary)] text-[var(--ui-secondary-foreground)] shadow-[var(--ui-shadow-sm)] hover:shadow-[var(--ui-shadow-md)]",
        tertiary:
          "bg-[var(--ui-accent)] text-[var(--ui-accent-foreground)] shadow-[var(--ui-shadow-md)] hover:shadow-[var(--ui-shadow-lg)]",
      },
      size: {
...
>
  {#if icon}
    <span
      class="flex shrink-0 items-center justify-center [&_svg]:h-6 [&_svg]:w-6"
    >
      {@render icon()}
    </span>
  {/if}
