...
<script lang="ts">
  ...
  const currentHeight = $derived(
    isCollapsible
      ? Math.round(expandedHeight - scrollThreshold * progress)
      : COLLAPSED_HEIGHT,
  );
  ...
  const titleFontSize = $derived(
    isCollapsible
      ? Math.round(expandedFontSize - (expandedFontSize - 16) * progress)
      : 16,
  );
...
>
  {#if isCollapsible}
    ...
    <div class="absolute inset-x-0 bottom-0 flex items-center px-4 pb-3">
      ...
      {#if actions}
        <div
          class="ml-auto shrink-0 flex items-center text-[var(--ui-on-surface)]"
        >
          {@render actions()}
        </div>
      {/if}
