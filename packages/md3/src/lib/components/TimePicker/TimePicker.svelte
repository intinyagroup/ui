...
>
  ...
  <div class="flex items-center justify-between w-full px-2">
    <div class="flex items-center gap-1">
      ...
      <span class="text-lg font-medium text-[var(--ui-on-surface-variant)]"
        >:</span
      >
      <button
        class={cn(
  ...
  {#if inputMode === "clock"}
    <!-- Clock Face -->
    <div
      class="relative w-64 h-64 rounded-full bg-[var(--ui-surface-container-lowest, var(--ui-background))]"
    >
      <!-- Clock face background circle -->
      <div
        class="absolute inset-2 rounded-full bg-[var(--ui-surface-container-low)]"
      ></div>

      <!-- Center dot -->
      <div
        class="absolute top-1/2 left-1/2 w-3 h-3 -ml-1.5 -mt-1.5 rounded-full bg-[var(--ui-primary)] z-20"
      ></div>
      ...
      {#each mode === "hours" ? hourNumbers : minuteNumbers as num, i}
        {@const total =
          mode === "hours" ? hourNumbers.length : minuteNumbers.length}
    ...
    <div class="flex flex-col items-center gap-4 py-8 w-full">
      <div class="relative">
        ...
        {#if keyboardValue.length > 0}
          <div
            class="absolute right-3 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-[var(--ui-primary)] animate-pulse"
          ></div>
        {/if}
      </div>
  ...
  {#if format === "12h"}
    <div
      class="flex gap-1 p-1 rounded-full bg-[var(--ui-surface-container-low)]"
    >
      <button
        class={cn(
