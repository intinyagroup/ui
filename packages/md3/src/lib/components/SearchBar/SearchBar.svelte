...
<div
  class={cn(
    ...
    // Focused state
    focused &&
      "border-[var(--ui-primary)] shadow-[0_0_0_1px_var(--ui-primary)]",
    // Hover state
...
>
  ...
  {#if leadingIcon}
    <span
      class="relative z-10 flex shrink-0 items-center justify-center text-[var(--ui-on-surface-variant)] [&_svg]:h-6 [&_svg]:w-6"
    >
      {@render leadingIcon()}
    </span>
  {/if}
