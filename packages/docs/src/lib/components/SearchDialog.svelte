...
{#if open}
  <div class="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div
      class="relative z-10 w-full max-w-lg rounded-xl border border-[var(--ui-border)] bg-[var(--ui-background)] shadow-2xl"
      onkeydown={handleKeydown}
    >
      <div
        class="flex items-center gap-3 border-b border-[var(--ui-border)] px-4"
      >
        ...
        {#if query}
          <button
            ...
            class="rounded p-1 hover:bg-[var(--ui-muted)]"
          >
            <X class="size-4" />
          </button>
        {/if}
        ...
        <div class="max-h-80 overflow-y-auto p-2">
          {#each results as result}
            ...
            >
              ...
              <span
                class="text-xs text-[var(--ui-muted-foreground)] line-clamp-2"
      ...
      {:else if query && !loading}
        <div class="p-6 text-center text-sm text-[var(--ui-muted-foreground)]">
          No results found.
        </div>
      {:else if !query}
        <div class="p-6 text-center text-sm text-[var(--ui-muted-foreground)]">
      ...
      {#if loading}
        <div class="p-4 text-center text-xs text-[var(--ui-muted-foreground)]">
          Searching...
        </div>
      {/if}
    </div>
