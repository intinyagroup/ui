...
<div class="flex flex-col gap-3">
  {#each schema as prop (prop.name)}
        ...
        {#if prop.min !== undefined && prop.max !== undefined}
          <div class="flex items-center gap-2">
            <input
              ...
              value={values[prop.name]}
              oninput={(e) =>
                onchange(prop.name, Number(e.currentTarget.value))}
    ...
    {:else if prop.type === "boolean"}
      <label class="flex cursor-pointer items-center justify-between gap-2">
        ...
        <button
          ...
          class="relative h-5 w-9 rounded-full transition-colors {values[
            prop.name
          ]
            ? 'bg-[var(--ui-primary)]'
        ...
        >
          <span
            class="absolute top-1/2 size-3.5 -translate-y-1/2 rounded-full bg-white shadow transition-all {values[
              prop.name
            ]
              ? 'left-[calc(100%-1rem)]'
              : 'left-[0.125rem]'}"
    ...
    {#if prop.description}
      <p
        class="-mt-1.5 text-[10px] leading-snug text-[var(--ui-muted-foreground)]/70"
      >
        {prop.description}
      </p>
    {/if}
