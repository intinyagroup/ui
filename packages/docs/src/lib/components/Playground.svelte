...
<div
  class="my-6 overflow-hidden rounded-lg border border-[var(--ui-border)] {className}"
>
  <!-- Header -->
  <div
    class="flex items-center justify-between gap-3 border-b border-[var(--ui-border)] px-4 py-2"
  >
    <div class="min-w-0">
      <span class="text-sm font-medium">{title}</span>
      {#if description}
        <p class="truncate text-xs text-[var(--ui-muted-foreground)]">
          {description}
        </p>
      {/if}
    </div>
    <div class="flex shrink-0 items-center gap-1">
      ...
      <button
        onclick={() => (tab = "preview")}
        class="flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs transition-colors {tab ===
        'preview'
  ...
  <div class="flex flex-col md:flex-row">
    <div
      class="flex min-h-32 flex-1 items-center justify-center overflow-x-auto p-8"
    >
      {#if tab === "preview"}
