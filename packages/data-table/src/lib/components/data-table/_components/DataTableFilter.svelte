    ...
    <div class="grid gap-3">
      ...
      <div class="grid gap-2">
        <Input
          ...
          onkeydown={(e) => {
            if (e.key === "Enter") applyTextFilter();
          }}
        />
      </div>
      ...
      {#if facetedValues.length > 0 && facetedValues.length <= 20}
        <div class="grid gap-1.5 max-h-48 overflow-auto">
          ...
          {#each facetedValues as value (String(value))}
            <label
              class="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-[var(--ui-foreground)] hover:bg-[var(--ui-secondary)] cursor-pointer"
            >
              <Checkbox
                checked={Array.isArray(filterValue) &&
