...
<div
  class="flex flex-wrap items-center gap-2 rounded-lg border border-dashed border-[var(--ui-border)] bg-[var(--ui-secondary)]/30 px-3 py-2 transition-colors {dragOver
...
>
  <span
    class="text-[11px] font-medium uppercase tracking-wider text-[var(--ui-muted-foreground)]"
    >Group by:</span
  >

  {#if grouping.length === 0}
    <span class="text-xs text-[var(--ui-muted-foreground)]/60"
      >Drag column here to group</span
    >
  {:else}
    {#each grouping as columnId (columnId)}
      <span
        class="inline-flex items-center gap-1.5 rounded-md bg-[var(--ui-primary)]/10 px-2.5 py-1 text-xs font-medium text-[var(--ui-primary)]"
      >
