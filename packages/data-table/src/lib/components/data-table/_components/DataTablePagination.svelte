...
<div
  class="flex flex-col gap-4 border-t border-[var(--ui-border)] bg-[var(--ui-secondary)]/35 px-5 py-4 text-xs font-medium text-[var(--ui-muted-foreground)] sm:flex-row sm:items-center sm:justify-between sm:px-6"
>
  <div class="flex items-center gap-3">
    <span
      class="inline-flex h-6 items-center rounded-md bg-[var(--ui-card)] px-2.5 text-[11px] font-semibold text-[var(--ui-foreground)] shadow-sm"
    >
      {#if rowCount === 0}
        0 rows
      {:else}
  ...
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-end">
    ...
    <div
      class="flex items-center gap-1 rounded-lg bg-[var(--ui-card)] p-1 shadow-sm"
    >
      <Button
        variant="ghost"
        size="sm"
      ...
      <span
        class="px-3 text-[11px] font-bold tracking-wider text-[var(--ui-foreground)]"
      >
