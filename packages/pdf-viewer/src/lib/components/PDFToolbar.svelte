...
<div
  class="flex items-center gap-1 px-3 py-2 border-b border-[var(--ui-border)] bg-[var(--ui-card)]"
>
  <!-- Sidebar toggles -->
  <Button
    variant="ghost"
  ...
  <div class="flex items-center gap-0.5 ml-1">
    <Button
      ...
      onclick={onZoomFitWidth}>Fit Width</Button
    >
    <Button
      variant="ghost"
      size="sm"
      class="h-7 px-2 text-[10px]"
      onclick={onZoomFitPage}>Fit Page</Button
    >
    <Button
      variant="ghost"
      size="sm"
      class="h-7 px-2 text-[10px]"
      onclick={onZoomActual}>100%</Button
    >
  </div>
  ...
  {#each tools as t (t.id)}
    <div class="relative">
      <Button
        ...
        class={cn(
          "size-8 p-0",
          tool === t.id && "bg-[var(--ui-primary)]/10 text-[var(--ui-primary)]",
        )}
  ...
  <div class="relative">
    ...
    >
      <div
        class="size-4 rounded border border-[var(--ui-border)]"
        style="background-color: {selectedAnnotationColor}"
      ></div>
    </button>
    {#if showColorPicker}
      <div
        class="absolute top-full right-0 z-50 mt-1 p-2 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)] shadow-lg grid grid-cols-4 gap-1"
      >
  ...
  {#if tool !== "select"}
    <Button
      variant="ghost"
      size="sm"
      class="size-8 p-0 text-[var(--ui-destructive)]"
  ...
  <div class="flex items-center gap-0.5">
    <Button
      ...
      onclick={() => onSidebarMode("thumbnails")}
      title="Thumbnails"
    >
    ...
    <Button
      ...
      onclick={() => onSidebarMode("bookmarks")}
      title="Bookmarks"
    >
    ...
    <Button
      ...
      onclick={() => onSidebarMode("annotations")}
      title="Annotations"
    >
      <Layers class="size-4" />
    </Button>
    <Button
      ...
      class={cn(
        "size-8 p-0",
        sidebarMode === "pages" && showSidebar && "bg-[var(--ui-secondary)]",
      )}
      onclick={() => {
        onSidebarMode("pages");
        onTogglePageManipulation();
