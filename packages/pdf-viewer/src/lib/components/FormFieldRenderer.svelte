...
<div
  class="absolute"
  style="left: {field.x * scale}px; top: {field.y *
    scale}px; width: {field.width * scale}px; height: {field.height * scale}px;"
>
  {#if field.type === "text"}
    <input
      type="text"
      ...
      style="font-size: {(field.fontSize ?? 12) *
        scale}px; text-align: {field.textAlign ?? 'left'};"
    />
  {:else if field.type === "checkbox"}
    <label class="flex items-center gap-1.5 h-full cursor-pointer">
      ...
      {#if field.alternateName}
        <span class="text-xs text-[var(--ui-foreground)]"
          >{field.alternateName}</span
        >
      {/if}
    </label>
  {:else if field.type === "radio"}
    <label class="flex items-center gap-1.5 h-full cursor-pointer">
      <input
        ...
        onchange={() =>
          onValueChange(field.id, field.exportValue ?? field.name)}
        disabled={field.readOnly}
        class="size-4 accent-[var(--ui-primary)]"
      />
      {#if field.alternateName}
  ...
  {:else if field.type === "listbox"}
    <select
      ...
      onchange={(e) =>
        handleSelectChange(
          Array.from(e.currentTarget.selectedOptions, (o) => o.value).join(","),
        )}
      disabled={field.readOnly}
  ...
  {:else}
    <div
      class="w-full h-full border border-[var(--ui-border)] rounded flex items-center justify-center"
    >
