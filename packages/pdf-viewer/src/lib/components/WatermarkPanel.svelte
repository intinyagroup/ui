    ...
    <div class="grid grid-cols-2 gap-2">
      <label class="block text-xs text-[var(--ui-muted-foreground)]">
        ...
        <input
          ...
          class="w-full h-8 mt-1 rounded cursor-pointer"
        />
      </label>
      <label class="block text-xs text-[var(--ui-muted-foreground)]">
        Font size
        <input
          ...
          min={12}
          max={200}
          class="w-full h-8 px-2 mt-1 rounded border border-[var(--ui-input)] text-sm"
        />
      </label>
    </div>
  ...
  {#if currentWatermarks.length > 0}
    <div class="space-y-1">
      <span class="text-xs text-[var(--ui-muted-foreground)]"
        >Active watermarks:</span
      >
      {#each currentWatermarks as wm}
        <div
          class="flex items-center justify-between text-sm px-2 py-1 rounded bg-[var(--ui-secondary)]/50"
        >
          <span>{wm.text ?? wm.type}</span>
          <button
            onclick={() => onRemoveWatermark(wm.id)}
