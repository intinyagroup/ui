...
<div class="space-y-4">
  ...
  {#if showForm}
    <div
      class="space-y-3 p-3 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-secondary)]/20"
    >
      ...
      <div class="text-xs text-[var(--ui-muted-foreground)] mb-1">
        Draw your signature:
      </div>
      <canvas
  ...
  {#if signatures.length > 0}
    <div class="space-y-2">
      <span class="text-xs text-[var(--ui-muted-foreground)]"
        >Signatures ({signatures.length}):</span
      >
      {#each signatures as sig (sig.id)}
        <div
          class="p-2 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)]"
        >
          <div class="flex items-start gap-2">
            <img
              src={sig.imageData}
              alt="Signature"
              class="h-10 border rounded bg-white"
            />
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-[var(--ui-foreground)]">
                {sig.signerName}
              </div>
