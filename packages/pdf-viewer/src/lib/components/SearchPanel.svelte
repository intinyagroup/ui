...
<div
  class="flex flex-col w-80 border-l border-[var(--ui-border)] bg-[var(--ui-card)]"
>
  <!-- Header -->
  <div
    class="flex items-center justify-between px-3 py-2 border-b border-[var(--ui-border)]"
  >
  ...
  <div class="p-3 space-y-2">
    ...
    {#if searchState.totalResults > 0}
      <div
        class="flex items-center justify-between text-xs text-[var(--ui-muted-foreground)]"
      >
        <span
          >{searchState.currentResult + 1} of {searchState.totalResults} results</span
        >
  ...
  {#if searchState.results.length > 0}
    <div class="flex-1 overflow-auto border-t border-[var(--ui-border)]">
      {#each searchState.results as result, index (result.pageNumber + "-" + result.index)}
        ...
        >
          <span
            class="text-[10px] px-1.5 py-0.5 rounded bg-[var(--ui-secondary)] text-[var(--ui-muted-foreground)] font-mono shrink-0"
          >
            p.{result.pageNumber}
          </span>
