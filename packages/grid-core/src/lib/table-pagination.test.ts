...
describe("table-pagination", () => {
  ...
  describe("getVisibleRowRange", () => {
    it("returns 0-0 when rowCount is 0", () => {
      expect(getVisibleRowRange(0, { pageIndex: 0, pageSize: 10 })).toEqual({
        from: 0,
        to: 0,
      });
    });
    ...
    it("calculates range on first page", () => {
      expect(getVisibleRowRange(45, { pageIndex: 0, pageSize: 10 })).toEqual({
        from: 1,
        to: 10,
      });
    });
    ...
    it("calculates range on last partial page", () => {
      expect(getVisibleRowRange(45, { pageIndex: 4, pageSize: 10 })).toEqual({
        from: 41,
        to: 45,
      });
    });
