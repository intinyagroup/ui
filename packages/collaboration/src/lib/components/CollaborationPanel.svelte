...
{#if showPanel}
  <div
    class="fixed right-0 top-0 bottom-0 w-80 border-l border-[var(--ui-border)] bg-[var(--ui-card)] shadow-xl z-50 flex flex-col"
  >
    <!-- Header -->
    <div
    ...
    >
      <div class="flex items-center gap-2">
        <div
          class="size-2.5 rounded-full {state.connected
            ? 'bg-[var(--ui-success)]'
            : 'bg-[var(--ui-destructive)]'}"
        ></div>
    ...
    <div class="flex border-b border-[var(--ui-border)]">
      {#each [{ id: "users", label: "Users", icon: Users, count: state.users.length }, { id: "comments", label: "Comments", icon: MessageSquare, count: state.comments.length }, { id: "versions", label: "History", icon: Clock, count: state.versions.length }] as tab}
        <button
        ...
        >
          ...
          {#if tab.count > 0}
            <span
              class="text-[10px] px-1.5 py-0.5 rounded-full bg-[var(--ui-secondary)]"
              >{tab.count}</span
            >
          {/if}
        </button>
      ...
      {#if activeTab === "users"}
        ...
        <div class="p-4 space-y-3">
          {#if state.users.length === 0}
            <div
              class="text-center py-8 text-sm text-[var(--ui-muted-foreground)]"
            >
          ...
          {:else}
            {#each state.users as user (user.id)}
              <div
                class="flex items-center gap-3 p-2 rounded-lg hover:bg-[var(--ui-secondary)]/50"
              >
                <div
      ...
      {:else if activeTab === "comments"}
        ...
        <div class="p-4 space-y-4">
          ...
          {#each state.comments as comment (comment.id)}
            <div
              class="p-3 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)]"
            >
              <div class="flex items-center gap-2 mb-2">
                <div
                  class="size-6 rounded-full bg-[var(--ui-primary)] flex items-center justify-center text-[10px] font-bold text-white"
                >
                  {getInitials(comment.userId)}
                </div>
                <span class="text-xs font-medium text-[var(--ui-foreground)]"
                  >{comment.userId.slice(0, 8)}</span
                >
                ...
                {#if comment.resolved}
                  <span
                    class="text-[10px] px-1.5 py-0.5 rounded bg-[var(--ui-success)]/10 text-[var(--ui-success)]"
                    >Resolved</span
                  >
                {/if}
              </div>
              <p class="text-sm text-[var(--ui-foreground)] mb-2">
                {comment.content}
              </p>
              ...
              {#if comment.replies.length > 0}
                <div
                  class="ml-6 space-y-2 border-l-2 border-[var(--ui-border)] pl-3"
                >
                  {#each comment.replies as reply}
                    <div>
                      <div class="flex items-center gap-2 mb-1">
                        <span class="text-xs font-medium"
                          >{reply.userId.slice(0, 8)}</span
                        >
                        <span
                          class="text-[10px] text-[var(--ui-muted-foreground)]"
        ...
        <div class="p-4 space-y-3">
          {#each state.versions as version (version.id)}
            ...
            >
              ...
              {/if}
              <div
                class="flex items-center justify-between text-[10px] text-[var(--ui-muted-foreground)]"
              >
                <span
                  >{version.createdBy} · {formatTime(version.createdAt)}</span
                >
