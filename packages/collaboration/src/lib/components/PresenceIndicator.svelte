...
<div class="flex items-center gap-2">
  ...
  <div class="flex -space-x-2">
    {#each state.users.slice(0, 5) as user (user.id)}
      ...
      >
        ...
        <div
          class="absolute bottom-0 right-0 size-2 rounded-full bg-[var(--ui-success)] border border-[var(--ui-card)]"
        ></div>
      </div>
    {/each}
    {#if state.users.length > 5}
      <div
        class="size-7 rounded-full flex items-center justify-center text-[10px] font-bold bg-[var(--ui-secondary)] text-[var(--ui-muted-foreground)] border-2 border-[var(--ui-card)]"
      >
  ...
  <div class="flex items-center gap-1">
    <div
      class="size-2 rounded-full {state.connected
        ? 'bg-[var(--ui-success)]'
