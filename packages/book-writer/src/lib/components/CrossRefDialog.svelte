...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
  <div
    class="bg-[var(--ui-card)] rounded-xl p-6 w-[480px] shadow-xl max-h-[80vh] overflow-auto"
  >
    ...
    <p class="text-sm text-[var(--ui-muted-foreground)] mb-4">
      Link to chapters, pages, or other elements. The reference updates
    ...
    <!-- Options list -->
    <div
      class="max-h-48 overflow-auto border border-[var(--ui-border)] rounded-lg mb-4"
    >
      ...
      {:else}
        {#each filteredOptions as option (option.value)}
          ...
          >
            <span
              class="text-[var(--ui-muted-foreground)] text-xs uppercase w-12"
              >{option.type}</span
            >
    ...
    {#if selectedTarget}
      <div
        class="p-3 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-secondary)]/20 mb-4"
      >
        <span class="text-xs text-[var(--ui-muted-foreground)]">Preview:</span>
        <span class="text-sm text-[var(--ui-primary)] ml-2">
          {customLabel ||
            options.find((o) => o.value === selectedTarget)?.label}
        </span>
      </div>
    {/if}
    ...
    <div class="flex justify-end gap-2">
      ...
      <Button onclick={handleInsert} disabled={!selectedTarget}
