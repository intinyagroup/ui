<script lang="ts">
  ...
  let exportFormat = $state<"pdf" | "epub" | "docx" | "markdown" | "print">(
    "pdf",
  );
  ...
  const formats: {
    ...
    },
    {
      ...
      label: "EPUB",
      icon: BookOpen,
      description: "E-book format (Kindle, Kobo)",
    },
    {
      value: "docx",
      label: "DOCX",
  ...
  const safeTitle = $derived(
    metadata.title.replace(/[^a-zA-Z0-9]/g, "_").toLowerCase() || "book",
  );
  ...
  async function exportToPDF() {
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      page.drawText(chapter.title, {
        x:
          layout.chapterTitleAlign === "center"
            ? size.width / 2 -
              fontBold.widthOfTextAtSize(
                chapter.title,
                layout.chapterTitleSize,
              ) /
                2
            : margin.left,
        y,
      ...
      if (plainText) {
        ...
        for (const word of words) {
          ...
          if (
            font.widthOfTextAtSize(testLine, layout.fontSize) > contentWidth
          ) {
            if (y < margin.bottom) {
              const p = pdfDoc.addPage([size.width, size.height]);
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          {#each formats as format (format.value)}
            ...
            >
              <Icon
                class="size-6 {exportFormat === format.value
                  ? 'text-[var(--ui-primary)]'
                  : 'text-[var(--ui-muted-foreground)]'}"
              />
              <span class="text-sm font-medium">{format.label}</span>
              <span
                class="text-[10px] text-[var(--ui-muted-foreground)] text-center"
        ...
        <!-- Book info -->
        <div
          class="text-xs text-[var(--ui-muted-foreground)] space-y-1 p-3 rounded-lg bg-[var(--ui-secondary)]/20"
        >
        ...
        {#if exporting}
          <div
            class="flex items-center gap-2 text-sm text-[var(--ui-primary)] p-3 rounded-lg bg-[var(--ui-primary)]/5"
          >
