  ...
  {#if pendingChanges.length > 0}
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-1">
          ...
          <Button
            variant="ghost"
            size="sm"
            class="h-6 px-2 text-[10px] text-[var(--ui-destructive)]"
            onclick={onRejectAll}
          >
      ...
      {#each pendingChanges as change (change.id)}
        <div
          class="p-2 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)] text-sm"
        >
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-xs text-[var(--ui-muted-foreground)]"
              >{truncateText(change.content)}</span
            >
          </div>
          <div class="flex items-center justify-between">
