<script lang="ts">
  ...
  const pageSize = $derived(
    getEffectivePageSize(layout.pageSize, layout.orientation ?? "portrait"),
  );
  ...
  async function repaginate() {
    ...
    for (const chapter of previewChapters) {
      ...
      const contentHtml =
        titleHtml + (chapter.content || "<p><em>Empty chapter</em></p>");
...
<div bind:this={containerEl} class="flex-1 flex flex-col overflow-hidden">
  <!-- View mode toggle -->
  <div
    class="flex items-center justify-between px-6 py-2 border-b border-[var(--ui-border)] bg-[var(--ui-card)]/50"
  >
  ...
  <div class="flex-1 overflow-auto p-6 bg-[var(--ui-muted)]/30">
            ...
            >
              {#if metadata.coverImage}
                <img
                  ...
                  alt="Cover"
                  class="w-32 h-auto mb-8 rounded shadow"
                />
              {:else}
                <div
                  class="w-20 h-20 rounded-full bg-[var(--ui-primary)]/10 flex items-center justify-center mb-8"
                >
    ...
    {:else}
      ...
      <div class="flex flex-col items-center gap-8">
        ...
        {#if layout.showCoverPage}
          <div
            class="bg-white shadow-xl border border-[var(--ui-border)]"
            style="width: {pageSize.width *
              0.75}px; aspect-ratio: {pageSize.width}/{pageSize.height}; padding: {layout.marginTop *
              0.75}px {layout.marginRight * 0.75}px {layout.marginBottom *
              0.75}px {layout.marginLeft * 0.75}px;"
          >
            <div
            ...
            >
              ...
              <h1
                ...
                style="font-size: {layout.chapterTitleSize * 0.75}pt;"
              >
                {layout.coverTitle || metadata.title}
              </h1>
              {#if layout.coverSubtitle || metadata.subtitle}
                <p
                  class="text-[var(--ui-muted-foreground)] mb-4"
                  style="font-size: {layout.fontSize * 0.75}pt;"
                >
                  {layout.coverSubtitle || metadata.subtitle}
                </p>
              {/if}
              <p
                class="text-[var(--ui-muted-foreground)]"
                style="font-size: {layout.fontSize * 0.6}pt;"
              >
                {metadata.author}
              </p>
            </div>
        ...
        {#each previewChapters as chapter (chapter.id)}
          ...
          {#if measuring}
            ...
            <div
              class="bg-white shadow-lg border border-[var(--ui-border)] flex items-center justify-center"
              style="width: {pageSize.width *
                0.75}px; height: {pageSize.height * 0.75}px;"
            >
              <span class="text-sm text-[var(--ui-muted-foreground)]"
                >Paginating "{chapter.title}"...</span
              >
            </div>
          {:else if pages.length > 0}
            {#each pages as page, i (page.pageNumber)}
              <div
                class="bg-white shadow-xl border {isActive && i === 0
                  ? 'border-[var(--ui-primary)]'
                  : 'border-[var(--ui-border)]'} relative"
                style="width: {pageSize.width *
                  0.75}px; height: {pageSize.height * 0.75}px;"
              >
                <!-- Page content -->
                <div
                  ...
                  style="padding: {layout.marginTop *
                    0.75}px {layout.marginRight * 0.75}px {layout.marginBottom *
                    0.75}px {layout.marginLeft *
                    0.75}px; font-family: {layout.fontFamily}; font-size: {layout.fontSize *
                    0.75}pt; line-height: {layout.lineHeight};"
                >
                ...
                {#if layout.showPageNumbers}
                  <div
                    class="absolute bottom-2 left-0 right-0 text-center text-[var(--ui-muted-foreground)]"
                ...
                <div
                  class="absolute top-2 right-3 text-[10px] text-[var(--ui-muted-foreground)]/40"
                >
                  p.{page.pageNumber}
                </div>
              </div>
          ...
          {:else}
            ...
            >
              <div class="text-center">
                ...
                <p class="text-sm text-[var(--ui-muted-foreground)] italic">
                  Empty chapter
                </p>
              </div>
            </div>
        ...
        {#if previewChapters.length === 0}
          <div
            class="bg-white shadow-xl rounded-sm p-8 text-center border border-[var(--ui-border)]"
          >
            <Eye
              class="size-8 mx-auto mb-3 text-[var(--ui-muted-foreground)]"
            />
            <p class="text-sm text-[var(--ui-muted-foreground)]">
              Add chapters to see a paginated preview
            </p>
          </div>
        {/if}
      </div>
