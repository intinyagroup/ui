...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
  <div
    class="bg-[var(--ui-card)] rounded-xl p-6 w-[560px] shadow-xl max-h-[80vh] overflow-auto"
  >
    ...
    <p class="text-sm text-[var(--ui-muted-foreground)] mb-4">
      Enter LaTeX math expressions. Use $...$ for inline math or $$...$$ for
      display math.
    </p>

    <!-- Templates -->
    <div class="mb-4">
      <span class="text-xs text-[var(--ui-muted-foreground)] mb-2 block"
    ...
    {#if previewHtml}
      <div
        class="p-4 rounded-lg border border-[var(--ui-border)] bg-white mb-4"
      >
        <span class="text-xs text-[var(--ui-muted-foreground)] block mb-2"
          >Preview:</span
        >
