<script lang="ts">
  ...
  function updateProp(key: string, value: any) {
    onUpdateSequence({
      ...sequence,
      props: { ...sequence.props, [key]: value },
    });
  }
  ...
  function updateStyle(key: string, value: any) {
    onUpdateSequence({
      ...sequence,
      style: { ...sequence.style, [key]: value },
    });
  }
  ...
  function removeEffect(effectId: string) {
    onUpdateSequence({
      ...sequence,
      effects: sequence.effects.filter((fx) => fx.id !== effectId),
    });
  }
</script>

<div
  class="w-64 border-l border-[var(--ui-border)] bg-[var(--ui-card)] flex flex-col overflow-hidden shrink-0"
>
  <div
    class="px-3 py-2 border-b border-[var(--ui-border)] flex items-center justify-between"
  >
    ...
    <div class="flex gap-1">
      ...
      <Button
        variant="ghost"
        size="sm"
        class="size-6 p-0 text-[var(--ui-destructive)]"
  ...
  <div class="flex-1 overflow-auto p-3 space-y-3 text-xs">
    ...
    <label class="block text-[var(--ui-muted-foreground)]">
      ...
      <input
        ...
        value={sequence.name}
        oninput={(e) =>
          onUpdateSequence({ ...sequence, name: e.currentTarget.value })}
      />
    </label>
    ...
    <div class="border-t border-[var(--ui-border)] pt-3">
      <span
        class="text-[10px] font-medium text-[var(--ui-muted-foreground)] uppercase"
        >Transform</span
      >
      <div class="grid grid-cols-2 gap-2 mt-2">
        ...
        <label class="text-[var(--ui-muted-foreground)]">
          ...
          <input
            type="number"
            class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
            value={sequence.style.width ?? 100}
        ...
        <label class="text-[var(--ui-muted-foreground)]">
          ...
          <input
            type="number"
            class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
            value={sequence.style.height ?? 100}
            oninput={(e) =>
              updateStyle("height", Number(e.currentTarget.value))}
          />
        </label>
        <label class="text-[var(--ui-muted-foreground)]">
          Rotation
          <input
            type="number"
            class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
            value={sequence.style.rotation ?? 0}
            oninput={(e) =>
              updateStyle("rotation", Number(e.currentTarget.value))}
          />
        </label>
        <label class="text-[var(--ui-muted-foreground)]">
          Scale
        ...
        <label class="text-[var(--ui-muted-foreground)]">
          ...
          <input
            ...
            max="1"
            class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
            value={sequence.style.opacity ?? 1}
            oninput={(e) =>
              updateStyle("opacity", Number(e.currentTarget.value))}
          />
        </label>
      </div>
    </div>
      ...
      <div class="border-t border-[var(--ui-border)] pt-3">
        ...
        <div class="grid gap-2 mt-2">
          <label class="text-[var(--ui-muted-foreground)]">
            Content
            <textarea
              class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs h-16 resize-none"
              value={sequence.props.content ?? ""}
          ...
          <div class="grid grid-cols-2 gap-2">
            <label class="text-[var(--ui-muted-foreground)]">
              ...
              <input
                type="number"
                class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
                value={sequence.props.fontSize ?? 48}
                oninput={(e) =>
                  updateProp("fontSize", Number(e.currentTarget.value))}
              />
            </label>
            <label class="text-[var(--ui-muted-foreground)]">
              Color
              <input
                ...
                value={sequence.props.color ?? "#ffffff"}
      ...
      <div class="border-t border-[var(--ui-border)] pt-3">
        ...
        <div class="grid gap-2 mt-2">
          <label class="text-[var(--ui-muted-foreground)]">
            ...
            <select
              class="w-full mt-1 px-2 py-1 rounded border border-[var(--ui-input)] text-xs"
              value={sequence.props.shape ?? "rectangle"}
          ...
          <label class="text-[var(--ui-muted-foreground)]">
            ...
            <input
              type="color"
              class="w-full mt-1 h-7 rounded cursor-pointer"
              value={sequence.props.fill ?? "#3b82f6"}
    ...
    {#if sequence.effects.length > 0}
      <div class="border-t border-[var(--ui-border)] pt-3">
        <span
          class="text-[10px] font-medium text-[var(--ui-muted-foreground)] uppercase"
          >Effects ({sequence.effects.length})</span
        >
        <div class="mt-2 space-y-1">
          {#each sequence.effects as fx}
            <div
              class="flex items-center justify-between px-2 py-1 rounded bg-[var(--ui-secondary)]/50"
            >
              <span class="text-xs">{fx.type}</span>
              <button
                onclick={() => removeEffect(fx.id)}
                class="text-[var(--ui-muted-foreground)] hover:text-[var(--ui-destructive)] cursor-pointer"
              >
                <Trash2 class="size-3" />
              </button>
            </div>
