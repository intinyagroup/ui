<script lang="ts">
  ...
  let composition = $state(
    initialComposition ?? createComposition({ name: "Untitled" }),
  );
  ...
  function handleTimeUpdate(frame: number) {
    currentFrame = frame;
  }

  function handleCompositionChange(comp: Composition) {
    composition = comp;
  }
  ...
  function addSequence(type: Sequence["type"]) {
    const seq = createSequence({
      ...
      props:
        type === "text"
          ? { content: "New Text", fontSize: 48, color: "#ffffff" }
          : type === "shape"
    ...
    composition = {
      ...composition,
      sequences: [...composition.sequences, seq],
    };
  ...
  function deleteSequence(id: string) {
    composition = {
      ...composition,
      sequences: composition.sequences.filter((s) => s.id !== id),
    };
  ...
  function duplicateSequence() {
    if (!selectedSequence) return;
    const dup = {
      ...JSON.parse(JSON.stringify(selectedSequence)),
      id: `seq-${Date.now()}`,
      name: `${selectedSequence.name} (copy)`,
    };
    composition = {
      ...composition,
      sequences: [...composition.sequences, dup],
    };
  }
</script>
...
<div class="flex flex-col h-screen bg-[var(--ui-background)]">
  <!-- Top toolbar -->
  <div
    class="flex items-center gap-3 px-4 py-2 border-b border-[var(--ui-border)] bg-[var(--ui-card)]"
  >
    <input
      type="text"
    ...
    <Button variant="outline" size="sm" onclick={() => onSave?.(composition)}
      >Save</Button
    >
  ...
  <div class="flex flex-1 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <!-- Preview -->
      <div
        class="flex-1 overflow-auto flex items-center justify-center p-4 bg-[var(--ui-muted)]/20"
      >
        <Player
          {composition}
          onFrameUpdate={handleTimeUpdate}
          onPlay={play}
          onPause={pause}
      ...
      <div
        class="flex items-center gap-2 px-4 py-2 border-t border-[var(--ui-border)]"
      >
      ...
      <!-- Timeline -->
      <div
        class="flex items-center gap-3 px-4 py-2 border-t border-[var(--ui-border)] bg-[var(--ui-card)]"
      >
        <Button
          variant="ghost"
          size="sm"
        ...
        <input
          ...
          value={currentFrame}
          oninput={(e) => {
            currentFrame = Number(e.currentTarget.value);
          }}
          class="flex-1 h-1 cursor-pointer"
        />
        <span
          class="text-[10px] font-mono text-[var(--ui-muted-foreground)] w-16 text-right"
        >
    ...
    {#if showInspector && selectedSequence}
      <InspectorPanel
        ...
        onUpdateSequence={(seq) => {
          composition = {
            ...composition,
            sequences: composition.sequences.map((s) =>
              s.id === seq.id ? seq : s,
            ),
          };
        }}
