<script lang="ts">
  ...
  import {
    getFilterString,
    getTransformString,
  } from "@intinyagroup/motion-effects";
  import {
  ...
  let {
    ...
    renderPoster?: Snippet<[{ isBuffering: boolean }]>;
    showPosterWhenUnplayed?: boolean;
    showPosterWhenPaused?: boolean;
    showPosterWhenEnded?: boolean;
    renderLoading?: Snippet<[{ height: number; width: number }]>;
    errorFallback?: Snippet<[{ error: Error }]>;
    renderPlayPauseButton?: Snippet<
      [{ playing: boolean; isBuffering: boolean }]
    >;
  ...
  const effectiveFrameEnd = $derived(
    outFrame ?? composition.durationInFrames - 1,
  );
  const effectiveDuration = $derived(
    effectiveFrameEnd - effectiveFrameStart + 1,
  );
  const progress = $derived(
    currentFrame / Math.max(1, composition.durationInFrames),
  );
  const currentTimecode = $derived(
    formatTimecode(currentFrame, composition.fps),
  );
  const totalTimecode = $derived(
    formatTimecode(composition.durationInFrames, composition.fps),
  );
  const scale = $derived(
    containerEl && canvas ? containerEl.clientWidth / composition.width : 1,
  );
  ...
  function emit(event: PlayerEvent, detail?: any) {
    (eventListeners[event] ?? []).forEach((cb) => cb(detail));
  }
  ...
  type PlayerAPI = {
    ...
    addEventListener: (e: PlayerEvent, cb: (d?: any) => void) => void;
  ...
  function render() {
    ...
    for (const seq of composition.sequences) {
      if (
        currentFrame < seq.from ||
        currentFrame >= seq.from + seq.durationInFrames
      )
        continue;
  ...
  function renderSequence(
    ...
    const x = (seq.style.x ?? 0) * (cw / 100),
      y = (seq.style.y ?? 0) * (ch / 100);
    const w = (seq.style.width ?? 100) * (cw / 100),
      h = (seq.style.height ?? 100) * (ch / 100);
  ...
  function animate(ts?: number) {
    ...
    if (ts && lastTimestamp) {
      ...
      const frames = Math.round(
        (delta / 1000) * composition.fps * currentPlaybackRate,
      );
  ...
  function seekTo(frame: number) {
    ...
    currentFrame = Math.max(
      effectiveFrameStart,
      Math.min(frame, effectiveFrameEnd),
    );
    render();
  ...
  function handleKeydown(e: KeyboardEvent) {
    if (spaceKeyToPlayOrPause && e.key === " ") {
      e.preventDefault();
  ...
  onMount(() => {
    ...
    onReady?.({
      ...
      isPlaying: () => isPlaying,
      getDuration: () => composition.durationMs,
      getFps: () => composition.fps,
      getScale: () => scale,
      getContainerNode: () => containerEl,
      mute: () => {
        isMuted = true;
        emit("mutechange", { isMuted: true });
      },
      unmute: () => {
        isMuted = false;
        emit("mutechange", { isMuted: false });
      },
      isMuted: () => isMuted,
      getVolume: () => volume,
      setVolume: (v) => {
...
>
  {#if error}
    ...
    {:else}<div
        class="flex items-center justify-center h-64 bg-red-950 text-red-200 text-sm"
      >
        Error: {error.message}
      </div>
    {/if}
  {:else}
    <canvas
      ...
      class="w-full h-auto {clickToPlay || doubleClickToFullscreen
        ? 'cursor-pointer'
        : ''}"
    ...
    {#if controls && showControlsTimer}
      ...
      >
        <div
          class="w-full h-2 bg-white/20 rounded-full cursor-pointer mb-3 hover:h-3 transition-all"
          onclick={(e) => {
            const r = (e.currentTarget as HTMLElement).getBoundingClientRect();
            seekTo(
              Math.round(
                effectiveFrameStart +
                  ((e.clientX - r.left) / r.width) * effectiveDuration,
              ),
            );
          }}
        >
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            {:else}
              <button
                onclick={() => (isPlaying ? pause() : play())}
                class="p-2 rounded-full bg-white/20 text-white hover:bg-white/30 cursor-pointer"
              >
                {#if isPlaying}<Pause class="size-5" />{:else}<Play
                    class="size-5"
                  />{/if}
              </button>
            {/if}
          ...
          <div class="text-sm font-mono text-white/80">
            <span class="text-white">{currentTimecode}</span><span
              class="text-white/40 mx-1">/</span
          ...
          <div class="flex items-center gap-2">
            {#if showPlaybackRateControl}
              ...
              >
                <option value={0.25}>0.25x</option><option value={0.5}
                  >0.5x</option
            ...
            {#if showVolumeControls}
              ...
              {:else}
                <button
                  ...
                  class="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 cursor-pointer"
                >
                  {#if isMuted}<VolumeX class="size-4" />{:else}<Volume2
                      class="size-4"
                    />{/if}
                </button>
              {/if}
            ...
            {#if allowFullscreen}
              {#if renderFullscreenButton}
                <button onclick={toggleFullscreen} class="cursor-pointer"
                  >{@render renderFullscreenButton({ isFullscreen })}</button
                >
              {:else}
                <button
                  ...
                  class="p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 cursor-pointer"
                >
                  {#if isFullscreen}<Minimize class="size-4" />{:else}<Maximize
                      class="size-4"
                    />{/if}
                </button>
              {/if}
