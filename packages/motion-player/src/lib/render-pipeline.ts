...
import { getFilterString } from "@intinyagroup/motion-effects";
...
export function renderFrame(
  ...
  for (const seq of composition.sequences) {
    ...
    ctx.save();
    const x = (seq.style.x ?? 0) * (width / 100),
      y = (seq.style.y ?? 0) * (height / 100);
    const w = (seq.style.width ?? 100) * (width / 100),
      h = (seq.style.height ?? 100) * (height / 100);
