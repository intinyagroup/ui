<script lang="ts">
  ...
  import { setCellValue, setCellStyle, createCell, type CellMap, type CellStyle } from '../cell-model.js';
  import { createSelectionState, normalizeRange, getCellsInRange, type CellRange, type SelectionState } from '../selection-store.js';
  ...
  function getSelectedBounds(): { startRow: number; startCol: number; endRow: number; endCol: number } {
  ...
  function applyStyleToSelection(style: Partial<CellStyle>) {
    ...
    let nextCells = activeSheet.cells;
    for (const id of cellIds) {
      nextCells = setCellStyle(nextCells, id, style);
    }
    updateActiveSheet((s) => ({
      ...s,
      cells: nextCells,
    ...
    undoRedo = pushUndo(undoRedo, { sheetId: workbook.activeSheetId, cellsSnapshot: prev, description: 'Format cells' });
  ...
  async function handlePaste() {
    ...
    for (let r = 0; r < lines.length; r++) {
      ...
      for (let c = 0; c < cols.length; c++) {
        ...
        nextCells = setCellValue(nextCells, getCellId(targetRow, targetCol), coerceValue(cols[c]));
      }
    }
    updateActiveSheet((s) => ({
      ...s,
    ...
    undoRedo = pushUndo(undoRedo, { sheetId: workbook.activeSheetId, cellsSnapshot: prev, description: 'Paste cells' });
  }
  ...
  function handleCut() {
    ...
    for (let r = startRow; r <= endRow; r++) {
      for (let c = startCol; c <= endCol; c++) {
        nextCells = setCellValue(nextCells, getCellId(r, c), null);
      }
    }
    updateActiveSheet((s) => ({
      ...s,
    ...
    undoRedo = pushUndo(undoRedo, { sheetId: workbook.activeSheetId, cellsSnapshot: prev, description: 'Cut cells' });
  }
  ...
  function handleGlobalKeydown(e: KeyboardEvent) {
    ...
    if (isCtrl && e.key === 'c' && !editingCellId) { e.preventDefault(); handleCopy(); }
    if (isCtrl && e.key === 'v' && !editingCellId) { e.preventDefault(); handlePaste(); }
    if (isCtrl && e.key === 'x' && !editingCellId) { e.preventDefault(); handleCut(); }
  }
...
<div class="flex flex-col h-full rounded-xl border border-[var(--ui-border)] bg-[var(--ui-card)] shadow-sm overflow-hidden {className ?? ''}">
  ...
  <!-- Formatting Toolbar -->
  <div class="flex items-center gap-1 border-b border-[var(--ui-border)] bg-[var(--ui-card)] px-3 py-1">
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('bold', true) && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
      onclick={() => applyStyleToSelection({ bold: !isStyleActive('bold', true) })}
      title="Bold (Ctrl+B)"
    >
      <Bold class="size-3.5" />
    </Button>
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('italic', true) && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
      onclick={() => applyStyleToSelection({ italic: !isStyleActive('italic', true) })}
      title="Italic (Ctrl+I)"
    >
    ...
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('underline', true) && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
      onclick={() => applyStyleToSelection({ underline: !isStyleActive('underline', true) })}
    ...
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('align', 'left') && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
    ...
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('align', 'center') && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
    ...
    <Button
      variant="ghost"
      size="sm"
      class={cn('size-7 p-0', isStyleActive('align', 'right') && 'bg-[var(--ui-secondary)] text-[var(--ui-foreground)]')}
