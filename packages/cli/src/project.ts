...
export function resolveProjectConfig(
  ...
  const defaults =
    framework === "react" ? DEFAULT_REACT_ALIASES : DEFAULT_SVELTE_ALIASES;
  return {
    root,
    framework,
    aliases: { ...defaults },
...
export function resolveAliasDir(config: ProjectConfig, alias: string): string {
  ...
  if (alias.startsWith("@/")) {
    ...
    return hasSrc
      ? join(config.root, "src", suffix)
      : join(config.root, suffix);
  }
