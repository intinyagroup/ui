...
const CATEGORIES = {
  ...
  utility: ["skeleton", "copy-to-clipboard", "emoji", "validate", "main"],
};
...
function extractProps(fileCode, variants) {
  ...
  const instance =
    scripts.find((s) => s.includes("$props()")) ?? scripts.at(-1);
  ...
  if (Array.isArray(exp.typeAnnotation)) {
    for (const pair of exp.typeAnnotation) {
      ...
      const c = firstColon(pair);
      const rawName =
        c === -1 ? pair.replace("?", "").trim() : pair.slice(0, c).trim();
...
for (const { package: pkg, dir: pkgDir } of PKG_DIRS) {
  ...
  for (const dir of readdirSync(pkgDir, { withFileTypes: true })) {
    ...
    for (const f of files) {
      contents[relative(pkgDir, f).split(sep).join("/")] = readFileSync(
        f,
        "utf8",
      );
    }
    ...
    let props = [];
    const propsSource =
      main ?? files.find((f) => isComponentFile(f)) ?? files.at(-1);
    if (propsSource) {
    ...
    components.push({
      ...
      mainTs: mainTs
        ? relative(pkgDir, mainTs).split(sep).join("/")
        : undefined,
