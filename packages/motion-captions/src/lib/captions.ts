...
export function createCaptionLine(
  text: string,
  startMs: number,
  endMs: number,
): CaptionLine {
...
export function getCaptionStyles(
  ...
  const progress = Math.min(
    1,
    (currentTimeMs - line.startMs) / (line.endMs - line.startMs),
  );
  let wordIndex = 0;
  for (let i = 0; i < line.words.length; i++) {
    if (
      currentTimeMs >= line.words[i].startMs &&
      currentTimeMs <= line.words[i].endMs
    ) {
      wordIndex = i;
      break;
    }
  ...
  switch (style.animation) {
    case "pop-in":
      anim = `transform: scale(${Math.min(1, progress * 2)}); opacity: ${Math.min(1, progress * 3)};`;
      break;
    case "slide-up":
      anim = `transform: translateY(${(1 - Math.min(1, progress * 2)) * 30}px); opacity: ${Math.min(1, progress * 3)};`;
      break;
    case "slide-down":
      anim = `transform: translateY(${(1 - Math.min(1, progress * 2)) * -30}px); opacity: ${Math.min(1, progress * 3)};`;
      break;
    case "typewriter":
      anim = `clip-path: inset(0 ${100 - ((progress * line.text.length) / line.text.length) * 100}% 0 0);`;
      break;
    case "highlight":
    case "karaoke":
      anim = "opacity: 1;";
...
export function renderCaptionHTML(
  ...
  for (let i = 0; i < line.words.length; i++) {
    ...
    const isPast = currentTimeMs > w.endMs;
    let ws = `color: ${style.color};`;
    if (style.animation === "highlight" && isCurrent)
      ws = `color: ${style.color}; font-weight: bold; text-shadow: 0 0 20px ${style.color};`;
    else if (style.animation === "karaoke")
      ws = isCurrent
        ? "color: #ffeb3b; font-weight: bold;"
        : isPast
          ? `color: ${style.color}; opacity: 0.5;`
          : `color: ${style.color}; opacity: 0.3;`;
