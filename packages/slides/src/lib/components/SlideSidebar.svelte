...
>
  <div class="px-3 py-2 border-b border-[var(--ui-border)]">
    ...
    <p class="text-[10px] text-[var(--ui-muted-foreground)]">
  ...
  <div class="flex-1 overflow-auto p-2 space-y-2">
    {#each slides as slide, index (slide.id)}
      ...
      >
        ...
        >
          <div
            class="w-full h-full p-2 text-[6px] leading-tight text-[var(--ui-foreground)]"
          >
            {#each slide.elements
              .filter((e) => e.type === "text")
              .slice(0, 3) as el}
              <div
                class:font-bold={el.style?.fontWeight === "bold"}
                style="font-size: {Math.max(
                  6,
                  (el.style?.fontSize ?? 12) / 6,
                )}px;"
              >
        ...
        <div
          class="flex items-center justify-between px-2 py-1.5 bg-[var(--ui-card)] rounded-b-md"
        >
          <span
            class="text-[10px] font-medium text-[var(--ui-muted-foreground)]"
          >
            {index + 1}
          </span>
          <div
          ...
          >
            <button
              onclick|stopPropagation={() => onDuplicateSlide(slide.id)}
            ...
            <button
              onclick|stopPropagation={() => onToggleHidden(slide.id)}
              class="p-0.5 rounded hover:bg-[var(--ui-secondary)] cursor-pointer"
              title={slide.isHidden ? "Show" : "Hide"}
            >
              {#if slide.isHidden}
                <EyeOff class="size-3" />
              {:else}
            ...
            {#if slides.length > 1}
              <button
                onclick|stopPropagation={() => onDeleteSlide(slide.id)}
                class="p-0.5 rounded hover:bg-[var(--ui-destructive)]/10 text-[var(--ui-destructive)] cursor-pointer"
                title="Delete"
              >
                <Trash2 class="size-3" />
              </button>
  ...
  <div class="p-2 border-t border-[var(--ui-border)]">
    <div class="relative">
      ...
      {#if showAddMenu}
        <div
          class="absolute bottom-full left-0 right-0 mb-1 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)] shadow-lg p-1 z-10 max-h-64 overflow-auto"
        >
          {#each slideLayouts as layout}
            <button
              onclick={() => {
                onAddSlide(layout.value);
                showAddMenu = false;
              }}
