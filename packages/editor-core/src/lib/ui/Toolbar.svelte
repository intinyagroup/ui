...
<div
  class="flex items-center gap-2 border-b border-[var(--ui-border)] bg-[var(--ui-card)] px-4 py-2"
>
  <button
    onclick={undo}
    disabled={!canUndo()}
  ...
  <button
    ...
    class="rounded p-2 hover:bg-[var(--ui-muted)] {$editor.showGrid
      ? 'text-[var(--ui-primary)]'
      : ''}"
    title="Toggle Grid"
  >
  ...
  <div class="relative">
    ...
    {#if showExport}
      <div
        class="absolute right-0 top-full mt-2 w-40 rounded-lg border border-[var(--ui-border)] bg-[var(--ui-card)] p-2 shadow-lg"
      >
        <button
          onclick={() => handleExport("svelte")}
          class="w-full rounded px-3 py-2 text-left text-sm hover:bg-[var(--ui-muted)]"
          >Svelte</button
        >
        <button
