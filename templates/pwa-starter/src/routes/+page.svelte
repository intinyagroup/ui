<script lang="ts">
  ...
  let toastVariant: "default" | "success" | "error" | "warning" =
    $state("success");
...
<svelte:head>
  <title>Intinya Mobile App</title>
</svelte:head>
...
<div class="mx-auto flex h-[100dvh] max-w-md flex-col overflow-hidden">
  ...
  >
    ...
    <span class="text-lg font-semibold text-[var(--ui-foreground)]"
      >Intinya App</span
    >
  </header>
  ...
  <main class="min-h-0 flex-1 overflow-y-auto">
    {#if activeTab === "home"}
      <div class="flex flex-col gap-4 p-4">
        ...
        <div class="flex items-center gap-2">
          <span class="text-xs font-medium text-[var(--ui-muted-foreground)]"
            >Team</span
          >
    ...
    {:else}
      <div class="p-4">
        <p class="mb-4 text-sm text-[var(--ui-muted-foreground)]">
          Profile placeholder
        </p>
        <ListView>
  ...
  {/snippet}
  <div
    class="pointer-events-none absolute inset-x-0 bottom-20 z-50 flex justify-end px-4"
  >
    <FabMenu
      items={[
        {
